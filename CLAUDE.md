...
- `app/_layout.tsx` — root `Stack`; awaits **both** `useAuthStore().laden()` and `useOnboardingStore().laden()` (in parallel) before hiding the splash screen. Declares route groups `onboarding`, `auth`, `(app)`, and `scan` (full-screen modal).
- `app/index.tsx` — entry gate: splash, then if `!onboardingAbgeschlossen` — redirect to `STATUS_META[session.status].route` if an in-progress `OnboardingSession` exists (mid-flow resume after a crash/close), else `/onboarding`; else if `!benutzer` redirect `/auth/otp`; else `/(app)/dashboard`.
...
- `app/auth/otp.tsx` — OTP send/verify mechanics extracted into `hooks/useOtpVerification.ts`; the screen itself is now **only** the re-login entry point for a returning user who is logged out (`onboardingAbgeschlossen === true && benutzer === null`). It is no longer part of the first-run flow — that requirement ("keine zusätzliche Anmeldung") is met by verifying the phone number inline inside Checkout instead (see below).
- `app/(app)/` — authenticated area with bottom tabs (`dashboard`, `einstellungen`); the tab bar has a floating center `CameraButton` that pushes `/scan/rezept` to start a **new** Versorgung for an already-logged-in user (not a tab). Note: this floating button's touch target currently overlaps the dashboard's "Details anzeigen" card below it — a pre-existing hit-testing quirk, not something introduced by the onboarding work, worth fixing if you're in this area.
- `app/scan/` — the full "start a Versorgung" flow, **reused for both first-time onboarding and repeat orders** (hence living under `scan/`, not `onboarding/`): `rezept` → `review` → `krankenkasse` → `datenpruefung` → `versorgungen` → `termin` → `zusammenfassung` → `checkout`. Every screen calls `useOnboardingGuard(status)` first. Mock OCR (`simuliereRezeptOcr`, `simuliereKrankenkasseOcr`) and the mock product catalog (`MOCK_PRODUKTE`) live in `lib/mockOcr.ts` — OCR is split across the two capture screens (prescription fields on `rezept.tsx`, insurance fields on `krankenkasse.tsx`) rather than one combined call, so `session.ocrResult` is progressively filled in. `checkout.tsx` skips the phone/OTP substep entirely and jumps straight to payment if `authStore.benutzer` already exists (the repeat-order case) — see `hooks/useOtpVerification.ts` for the shared send/verify mechanics used by both `otp.tsx` and `checkout.tsx`.
